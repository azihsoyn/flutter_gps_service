...
class LocationService : Service(), GpsStatus.Listener {
    ...
    override fun onCreate() {
        Log.v("hoge", "LocationService.onCreate called")
        ...
        val locationCallback = object : LocationCallback() {
            override fun onLocationResult(locationResult: LocationResult?) {
                ...
                Log.d("hoge", "緯度:${location.latitude}, 経度:${location.longitude}")
            }
        }

        fusedLocationClient.requestLocationUpdates(locationRequest, locationCallback, Looper.myLooper())

        val notificationActionReceiver = object : BroadcastReceiver(){
            override fun onReceive(ctx: Context, intent: Intent) {
                Log.d("hoge", "receiveIntent onReceive")
                val notificationId = intent.getIntExtra(EXTRA_NOTIFICATION_ID, 0)
                if(intent.action == "PAUSE"){
                    Log.d("hoge", "receiveIntent PAUSE")
                    val channelId =
                    ...
                    val pauseAction = Intent("PAUSE").apply {
                        putExtra(EXTRA_NOTIFICATION_ID, 101)
                    }
                    val resumeAction = Intent("RESUME").apply {
                        putExtra(EXTRA_NOTIFICATION_ID, 101)
                    }
                    val stopAction = Intent("STOP").apply {
                        putExtra(EXTRA_NOTIFICATION_ID, 101)
                    }
                    val pausePendingIntent: PendingIntent =
                            PendingIntent.getBroadcast(ctx, 0, pauseAction, PendingIntent.FLAG_UPDATE_CURRENT)
                    val resumePendingIntent: PendingIntent =
                            PendingIntent.getBroadcast(ctx, 0, resumeAction, PendingIntent.FLAG_UPDATE_CURRENT)
                    val stopPendingIntent: PendingIntent =
                            PendingIntent.getBroadcast(ctx, 0, stopAction, PendingIntent.FLAG_UPDATE_CURRENT)
                    val notificationBuilder = NotificationCompat.Builder(ctx, channelId )
                    val notification = notificationBuilder.setOngoing(true)
                            //.setSmallIcon(R.mipmap.ic_launcher)
                            .setPriority(PRIORITY_MIN)
                            .setCategory(Notification.CATEGORY_SERVICE)
                            .addAction(R.drawable.notification_icon_background, "再開", resumePendingIntent)
                            .addAction(R.drawable.design_ic_visibility_off, "終了", stopPendingIntent)
                            .build()
                    startForeground(notificationId, notification)
                }else if (intent.action == "RESUME"){
                    Log.d("hoge", "receiveIntent RESUME")
                    val channelId =
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                                createNotificationChannel("my_service", "Location Tracking Service")
                            } else {
                    ...
                    val notification = notificationBuilder.setOngoing(true)
                            ...
                            .setPriority(PRIORITY_MIN)
                            .setCategory(Notification.CATEGORY_SERVICE)
                            .addAction(R.drawable.notification_icon_background, "一時停止", pausePendingIntent)
                            .addAction(R.drawable.design_ic_visibility_off, "終了", stopPendingIntent)
                            .build()
                    startForeground(notificationId, notification)
                }else if (intent.action == "STOP"){
                    Log.d("hoge", "receiveIntent STOP")
                }
            }
        }

        LocalBroadcastManager.getInstance(applicationContext).registerReceiver(batteryInfoReceiver, IntentFilter(Intent.ACTION_BATTERY_CHANGED))
        registerReceiver(notificationActionReceiver, IntentFilter().apply {
            addAction("STOP")
            addAction("PAUSE")
            addAction("RESUME")
        })
        Log.d("hoge", "registered receiver")
    }


    override fun onStartCommand(i: Intent, flags: Int, startId: Int): Int {
        Log.v("hoge", "onStartCommand")
    ...
    fun startLogging() {
        Log.d("hoge", "startLogging")
        isLogging = true
    }


    fun stopLogging() {
        Log.d("hoge", "stopLogging")
    ...
    private fun startForeground() {
        ...
        val pausePendingIntent: PendingIntent =
                PendingIntent.getBroadcast(this, 0, pauseAction, PendingIntent.FLAG_UPDATE_CURRENT)
        val resumePendingIntent: PendingIntent =
                PendingIntent.getBroadcast(this, 0, resumeAction, PendingIntent.FLAG_UPDATE_CURRENT)
        val stopPendingIntent: PendingIntent =
                PendingIntent.getBroadcast(this, 0, stopAction, PendingIntent.FLAG_UPDATE_CURRENT)
